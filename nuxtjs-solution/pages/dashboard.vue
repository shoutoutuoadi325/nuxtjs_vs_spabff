      ...
      <div v-else-if="data?.success">
            ...
            <div class="p-5">
              <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                  ...
                  <dd class="text-lg font-medium text-gray-900">¥{{ data.data.stats.totalRevenue.toLocaleString() }}</dd>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
          <div class="px-4 py-5 sm:px-6">
            ...
            <p class="mt-1 max-w-2xl text-sm text-gray-500">聚合的用户和订单数据</p>
          </div>
        ...
        <div class="mt-6 bg-blue-50 border border-blue-200 rounded-md p-4">
          ...
          <ul class="text-sm text-blue-800 space-y-1">
            <li>• 使用 <code class="bg-blue-100 px-1 rounded">Promise.all()</code> 并行调用 User 和 Order 两个 gRPC 服务</li>
            <li>• 在服务端进行数据聚合，减少客户端网络请求</li>
            <li>• 将用户信息和订单信息组合成完整的业务数据</li>
            <li>• 计算统计数据（总用户数、总订单数、总收入等）</li>
            <li>• 一次 API 调用返回完整的仪表盘数据，提升性能</li>
          </ul>
        </div>
      </div>
    </div>
